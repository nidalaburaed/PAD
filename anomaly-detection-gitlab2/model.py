...
def predict_metrics(pd_dict, prediction_range=180):
    '''
    ...
    for meta_data in pd_dict:
        try:
            ...
            m = Prophet(growth='linear', seasonality_mode='additive', interval_width=0.99, changepoint_range=0.8, daily_seasonality = True, weekly_seasonality=True, yearly_seasonality=True)

            print("Fitting the train_frame")
            m.fit(train_frame)

            future = m.make_future_dataframe(periods=int(PREDICT_DURATION),freq="30s")
            ...
            #df_cv = cross_validation(m, horizon='1 hour')
...
def get_forecast_df_for_plot():

    return forecast


def graph_forecast_prophet():
...
def graph_forecast_prophet_components():
...
if __name__ == "__main__":
    ...
    data_size_hour = str(os.getenv('DATA_SIZE_HOUR','1d'))
